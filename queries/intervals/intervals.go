...
//   - The two analyzedText overloads are named AnalyzedText (Analyzer form) and
//     AnalyzedTextStream (TokenStream form), Go having no overloading.
...
// AnalyzedText returns intervals that correspond to tokens from a TokenStream
// returned for text by applying the provided Analyzer as if text was the content
// of the given field. The intervals can be ordered or unordered and can have
// optional gaps inside.
//
// Mirrors Intervals.analyzedText(String, Analyzer, String, int, boolean).
func AnalyzedText(text string, analyzer analysis.Analyzer, field string, maxGaps int, ordered bool) (IntervalsSource, error) {
	ts, err := analyzer.TokenStream(field, strings.NewReader(text))
	if err != nil {
		return nil, err
	}
	src, err := AnalyzedTextStream(ts, maxGaps, ordered)
	if closeErr := ts.Close(); closeErr != nil && err == nil {
		return nil, closeErr
	}
	return src, err
}

// AnalyzedTextStream returns intervals that correspond to tokens from the provided
// TokenStream. This is a low-level counterpart to AnalyzedText. The intervals can be
// ordered or unordered and can have optional gaps inside.
//
// Mirrors Intervals.analyzedText(TokenStream, int, boolean).
func AnalyzedTextStream(tokenStream analysis.TokenStream, maxGaps int, ordered bool) (IntervalsSource, error) {
	stream, ok := tokenStream.(*analysis.CachingTokenFilter)
	if !ok {
		stream = analysis.NewCachingTokenFilter(tokenStream)
	}
