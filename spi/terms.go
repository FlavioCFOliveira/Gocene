...
type Terms interface {
	...
	// Intersect returns a TermsEnum that iterates over terms accepted by the automaton,
	// starting at or after the given term.
	Intersect(compiled *automaton.CompiledAutomaton, startTerm *Term) (TermsEnum, error)
...
func (t *TermsBase) Field() string {
	return t.field
}

// Size returns -1 by default (unknown size).
func (t *TermsBase) Intersect(compiled *automaton.CompiledAutomaton, startTerm *Term) (TermsEnum, error) {
	return nil, fmt.Errorf("Intersect not implemented for TermsBase")
}
...
// GetMin returns nil.
func (e *EmptyTerms) Intersect(compiled *automaton.CompiledAutomaton, startTerm *Term) (TermsEnum, error) {
	return &EmptyTermsEnum{}, nil
}
...
func (s *SingleTermTerms) Intersect(compiled *automaton.CompiledAutomaton, startTerm *Term) (TermsEnum, error) {
	if s.term == nil {
		return &EmptyTermsEnum{}, nil
	}
	if compiled.Run(s.term.BytesValue()) {
		return NewSingleTermsEnum(s.term, s.docFreq, s.totalFreq), nil
	}
	return &EmptyTermsEnum{}, nil
}
