...
// KnnByteVectorField. It is the Go port of
// org.apache.lucene.index.ByteVectorValues from Apache Lucene 10.5.0; see
// [KnnVectorValues] for why the declaration lives in spi. The static members
// checkField and fromBytes need index types and are rendered in the index
// package as CheckByteVectorField and FromBytes.
type ByteVectorValues interface {
	...
	// CopyByteVectorValues is the covariant ByteVectorValues.copy()
	// override: it returns the same copy as Copy, typed as
	// ByteVectorValues.
	CopyByteVectorValues() (ByteVectorValues, error)

	// Scorer returns a VectorScorer for the given query vector. When the
	// underlying format quantizes the vectors, the scorer scores against the
	// quantized vectors. The result may be nil. Java's default body throws
