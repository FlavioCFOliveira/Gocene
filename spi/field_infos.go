...
type FieldInfos struct {
	...
	// through Add, so Freeze reproduces the point at which Java's
	// Builder.assertNotFinished() starts rejecting further additions.
	frozen bool
}
...
// Fields returns all FieldInfo objects in ascending order of field number.
func (fi *FieldInfos) Fields() []*FieldInfo {
	return fi.values
}

// FieldInfo returns the FieldInfo with the given name, or nil when this
// FieldInfos has no such field. Port of FieldInfos.fieldInfo(String)
// (FieldInfos.java:330).
//
...
// FieldInfos has no such field. It is the Gocene alias of
// FieldInfos.fieldInfo(String) (FieldInfos.java:330) and delegates to
// FieldInfoByName.
func (fi *FieldInfos) GetByName(name string) *FieldInfo {
	return fi.FieldInfoByName(name)
}

// GetByNumber returns the FieldInfo with the given field number, or nil when
// this FieldInfos has no such field. It is the Gocene alias of
// FieldInfos.fieldInfo(int) (FieldInfos.java:341) and delegates to
// FieldInfoByNumber.
func (fi *FieldInfos) GetByNumber(number int) *FieldInfo {
	return fi.FieldInfoByNumber(number)
}

// Infos returns all FieldInfo objects in ascending order of field number.
// It delegates to Fields and exposes the same backing slice that Iterator
// walks, which is the collection Java iterates through FieldInfos.iterator()
// (FieldInfos.java:321).
func (fi *FieldInfos) Infos() []*FieldInfo {
	return fi.Fields()
}

// Names returns the names of every field in this FieldInfos, in ascending
// order of field number.
//
// PORT NOTE: Java has no such accessor; callers iterate FieldInfos directly
// and read FieldInfo.name. This helper enumerates exactly the same fields in
// exactly the same order as FieldInfos.iterator() (FieldInfos.java:321), so it
// carries no behaviour of its own.
func (fi *FieldInfos) Names() []string {
	names := make([]string, 0, len(fi.values))
	for _, info := range fi.values {
		names = append(names, info.Name())
	}
	return names
}

// GetNextFieldNumber returns the lowest field number that is not yet assigned
// within this FieldInfos.
//
// PORT NOTE: Java assigns field numbers from FieldInfos.FieldNumbers, whose
// addOrGet advances lowestUnassignedFieldNumber until it finds a number absent
// from numberToName (FieldInfos.java:430). This method performs the same search
// against this FieldInfos' own number table, for the callers that have to place
// a field inside a single segment's FieldInfos rather than in the index-wide
// registry. It only reports the number; the field is reserved when it is passed
// to Add.
func (fi *FieldInfos) GetNextFieldNumber() int {
	number := 0
	for number < len(fi.byNumber) && fi.byNumber[number] != nil {
		number++
	}
	return number
}

// Freeze ends the mutation phase of this FieldInfos: every later Add is
// rejected.
//
// PORT NOTE: Lucene's FieldInfos is immutable from construction, and it is
// FieldInfos.Builder that refuses additions once finish() has run
// (Builder.assertNotFinished, FieldInfos.java:819). Gocene's FieldInfos is
// mutable through Add and so plays both roles; Freeze marks the transition,
// giving Add the same "already finished" contract Java gives Builder.add.
func (fi *FieldInfos) Freeze() {
	fi.frozen = true
}

// IsFrozen reports whether Freeze has been called on this FieldInfos, which is
// the Gocene equivalent of Java's FieldInfos.Builder.finished flag
// (FieldInfos.java:704).
...
//
// Adding to a frozen FieldInfos panics, mirroring the IllegalStateException
// Java raises from FieldInfos.Builder.assertNotFinished (FieldInfos.java:819).
func (fi *FieldInfos) Add(info *FieldInfo) *FieldInfo {
	if fi.frozen {
		panic("FieldInfos was already frozen; cannot add new fields")
	}
...
type fieldVectorProps struct {
	numDimensions            int
	vectorEncoding           util.VectorEncoding
	vectorSimilarityFunction util.VectorSimilarityFunction
}
