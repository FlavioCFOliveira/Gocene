...
- **ICU4J binary dictionaries: loader present, blobs not bundled.**
  `analysis/icu` implements the ICU Unicode segmentation pipeline and ships
  a parser for the compiled `.brk` format (`segmentation.LoadBRKDictionary`,
  `segmentation.ParseBRKDictionary`). The `Default.brk` and
  `MyanmarSyllable.brk` blobs themselves are not bundled — they are
  generated by an external ICU build pipeline — so by default
  CJK/Thai/Myanmar segmentation falls back to the rules-based UAX#29
  approximation. Procurement, bundling, and RBBI execution-engine work are
  tracked in the backlog; see `analysis/icu/README.md` for details.
