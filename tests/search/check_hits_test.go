...
// recordTB is a recording stub satisfying testsearch.TB so the tests can
...
func buildIndex(t *testing.T) (*search.IndexSearcher, func()) {
	...
	// the segment would read back empty.
	riw, err := testindex.NewRandomIndexWriterWithConfig(rand.New(rand.NewSource(42)), dir, cfg)
	if err != nil {
...
func TestCheckHits_MatchingSet(t *testing.T) {
	...
	rt := &recordTB{}
	testsearch.CheckHits(rt, q, "field", searcher, []int{0, 2, 4})
	if rt.failed() {
	...
	rt = &recordTB{}
	testsearch.CheckHits(rt, q, "field", searcher, []int{0, 1})
	if !rt.failed() {
...
func TestCheckDocIds_Order(t *testing.T) {
	...
	rt := &recordTB{}
	testsearch.CheckDocIds(rt, "ok", []int{3, 7}, hits)
	if rt.failed() {
		t.Fatalf("CheckDocIds flagged a correct order: %v", rt.errs)
	}

	rt = &recordTB{}
	testsearch.CheckDocIds(rt, "bad", []int{7, 3}, hits)
	if !rt.failed() {
		t.Fatalf("CheckDocIds did not flag a wrong order")
	}

	rt = &recordTB{}
	testsearch.CheckDocIds(rt, "len", []int{3}, hits)
	if !rt.failed() {
...
func TestCheckEqual_ScoresAndDocs(t *testing.T) {
	...
	rt = &recordTB{}
	testsearch.CheckEqual(rt, q, a, bTiny)
	if rt.failed() {
...
func TestCheckHitsQuery_Combined(t *testing.T) {
	...
	rt := &recordTB{}
	testsearch.CheckHitsQuery(rt, q, top.ScoreDocs, top.ScoreDocs, results)
	if rt.failed() {
...
func TestCheckExplanations_TermQuery(t *testing.T) {
	...
	rt := &recordTB{}
	testsearch.CheckExplanations(rt, q, "field", searcher, false)
	if rt.failed() {
...
func TestCheckNoMatchExplanations_TermQuery(t *testing.T) {
	...
	rt := &recordTB{}
	testsearch.CheckNoMatchExplanations(rt, q, "field", searcher, []int{0, 2, 4})
	if rt.failed() {
	...
	rt = &recordTB{}
	testsearch.CheckNoMatchExplanations(rt, q, "field", searcher, []int{0})
	if !rt.failed() {
...
func TestVerifyExplanation_SumOfDeep(t *testing.T) {
	...
	rt := &recordTB{}
	testsearch.VerifyExplanation(rt, "q", 0, 5.0, true, root)
	if rt.failed() {
	...
	rt = &recordTB{}
	testsearch.VerifyExplanation(rt, "q", 0, 4.0, false, root)
	if !rt.failed() {
	...
	rt = &recordTB{}
	testsearch.VerifyExplanation(rt, "q", 0, 5.0, true, bad)
	if !rt.failed() {
...
func TestVerifyExplanation_ProductOfDeep(t *testing.T) {
	...
	rt := &recordTB{}
	testsearch.VerifyExplanation(rt, "q", 0, 6.0, true, root)
	if rt.failed() {
...
func TestHits2StrAndTopDocsString(t *testing.T) {
	...
	s := testsearch.Hits2Str(hits, hits, 0, 0)
	...
	ts := testsearch.TopDocsString(td, 0, 0)
...
func TestQueryString(t *testing.T) {
	q := search.NewTermQuery(index.NewTerm("field", "aaa"))
	s := testsearch.QueryString(q, "field")
	if s == "" {
