...
func simpleExplanationScenarios(tc *explanationTestCase) []simpleExplanationScenario {
	dmq := func(tie float32, qs ...search.Query) search.Query {
		return search.NewDisjunctionMaxQuery(qs, tie)
	}
	...
	bq := func(build func(q *search.BooleanQueryBuilder)) search.Query {
	...
	return []simpleExplanationScenario{
		...
		{"P1", search.NewPhraseQuery(0, explField, "w1", "w2"), []int{0}},
		{"P2", search.NewPhraseQuery(0, explField, "w1", "w3"), []int{1, 3}},
		...
		{"BQ1", bq(func(q *search.BooleanQueryBuilder) {
			q.Add(simpleTerm("w1"), search.MUST)
			q.Add(simpleTerm("w2"), search.MUST)
		}), []int{0, 1, 2, 3}},
		{"BQ2", bq(func(q *search.BooleanQueryBuilder) {
			q.Add(simpleTerm("yy"), search.MUST)
			q.Add(simpleTerm("w3"), search.MUST)
		}), []int{2, 3}},
		{"BQ3", bq(func(q *search.BooleanQueryBuilder) {
			q.Add(simpleTerm("yy"), search.SHOULD)
			q.Add(simpleTerm("w3"), search.MUST)
		}), []int{0, 1, 2, 3}},
		{"BQ4", bq(func(q *search.BooleanQueryBuilder) {
			...
			q.Add(inner.Build(), search.SHOULD)
		}), []int{0, 1, 2, 3}},
		{"BQ5", bq(func(q *search.BooleanQueryBuilder) {
			q.Add(simpleTerm("w1"), search.SHOULD)
			inner := search.NewBooleanQueryBuilder()
		...
		{"BQ6", bq(func(q *search.BooleanQueryBuilder) {
			q.Add(simpleTerm("w1"), search.SHOULD)
			inner := search.NewBooleanQueryBuilder()
			inner.Add(simpleTerm("qq"), search.MUST_NOT)
			inner.Add(simpleTerm("w5"), search.SHOULD)
			q.Add(inner.Build(), search.MUST_NOT)
		}), []int{1, 2, 3}},
		{"BQ7", bq(func(q *search.BooleanQueryBuilder) {
			q.Add(simpleTerm("w1"), search.MUST)
			inner := search.NewBooleanQueryBuilder()
			inner.Add(simpleTerm("qq"), search.SHOULD)
			cl := search.NewBooleanQueryBuilder()
			cl.Add(simpleTerm("xx"), search.SHOULD)
			cl.Add(simpleTerm("w2"), search.MUST_NOT)
			inner.Add(cl.Build(), search.SHOULD)
			cr := search.NewBooleanQueryBuilder()
			cr.Add(simpleTerm("w3"), search.MUST)
			cr.Add(simpleTerm("w4"), search.MUST)
			inner.Add(cr.Build(), search.SHOULD)
			q.Add(inner.Build(), search.MUST)
		}), []int{0}},
		{"BQ8", bq(func(q *search.BooleanQueryBuilder) {
			q.Add(simpleTerm("w1"), search.MUST)
			inner := search.NewBooleanQueryBuilder()
			inner.Add(simpleTerm("qq"), search.SHOULD)
			cl := search.NewBooleanQueryBuilder()
		...
		{"BQ9", bq(func(q *search.BooleanQueryBuilder) {
			q.Add(simpleTerm("w1"), search.MUST)
			inner := search.NewBooleanQueryBuilder()
			inner.Add(simpleTerm("qq"), search.SHOULD)
			cl := search.NewBooleanQueryBuilder()
			...
			cr.Add(simpleTerm("w3"), search.MUST)
			cr.Add(simpleTerm("w4"), search.MUST)
			inner.Add(cr.Build(), search.MUST_NOT)
			q.Add(inner.Build(), search.SHOULD)
		}), []int{0, 1, 2, 3}},
		{"BQ10", bq(func(q *search.BooleanQueryBuilder) {
			q.Add(simpleTerm("w1"), search.MUST)
			inner := search.NewBooleanQueryBuilder()
			inner.Add(simpleTerm("qq"), search.SHOULD)
			cl := search.NewBooleanQueryBuilder()
		...
		{"BQ11", bq(func(q *search.BooleanQueryBuilder) {
			q.Add(simpleTerm("w1"), search.SHOULD)
			q.Add(search.NewBoostQuery(simpleTerm("w1"), 1000), search.SHOULD)
		}), []int{0, 1, 2, 3}},
		{"BQ14", bq(func(q *search.BooleanQueryBuilder) {
			q.Add(simpleTerm("QQQQQ"), search.SHOULD)
			q.Add(simpleTerm("w1"), search.SHOULD)
		}), []int{0, 1, 2, 3}},
		{"BQ15", bq(func(q *search.BooleanQueryBuilder) {
			q.Add(simpleTerm("QQQQQ"), search.MUST_NOT)
			q.Add(simpleTerm("w1"), search.SHOULD)
		}), []int{0, 1, 2, 3}},
		{"BQ16", bq(func(q *search.BooleanQueryBuilder) {
			q.Add(simpleTerm("QQQQQ"), search.SHOULD)
			inner := search.NewBooleanQueryBuilder()
		...
		{"BQ17", bq(func(q *search.BooleanQueryBuilder) {
		...
		{"BQ19", bq(func(q *search.BooleanQueryBuilder) {
			q.Add(simpleTerm("yy"), search.MUST_NOT)
			q.Add(simpleTerm("w3"), search.SHOULD)
		}), []int{0, 1}},
		{"BQ20", bq(func(q *search.BooleanQueryBuilder) {
		...
		{"BQ21", bq(func(q *search.BooleanQueryBuilder) {
			q.Add(simpleTerm("yy"), search.SHOULD)
			q.Add(simpleTerm("zz"), search.SHOULD)
		}), []int{1, 2, 3}},
		{"BQ23", bq(func(q *search.BooleanQueryBuilder) {
			q.Add(simpleTerm("w1"), search.FILTER)
			q.Add(simpleTerm("w2"), search.FILTER)
		}), []int{0, 1, 2, 3}},
		{"BQ24", bq(func(q *search.BooleanQueryBuilder) {
			q.Add(simpleTerm("w1"), search.FILTER)
			q.Add(simpleTerm("w2"), search.SHOULD)
		}), []int{0, 1, 2, 3}},
		{"BQ25", bq(func(q *search.BooleanQueryBuilder) {
			q.Add(simpleTerm("w1"), search.FILTER)
			q.Add(simpleTerm("w2"), search.MUST)
		}), []int{0, 1, 2, 3}},
		{"BQ26", bq(func(q *search.BooleanQueryBuilder) {
			q.Add(simpleTerm("w1"), search.FILTER)
			q.Add(simpleTerm("xx"), search.MUST_NOT)
		}), []int{0, 1}},
		{"MultiFieldBQ1", bq(func(q *search.BooleanQueryBuilder) {
			q.Add(simpleTerm("w1"), search.MUST)
			q.Add(simpleAltTerm("w2"), search.MUST)
		}), []int{0, 1, 2, 3}},
		{"MultiFieldBQ2", bq(func(q *search.BooleanQueryBuilder) {
			q.Add(simpleTerm("yy"), search.MUST)
			q.Add(simpleAltTerm("w3"), search.MUST)
		}), []int{2, 3}},
		{"MultiFieldBQ3", bq(func(q *search.BooleanQueryBuilder) {
			q.Add(simpleTerm("yy"), search.SHOULD)
			q.Add(simpleAltTerm("w3"), search.MUST)
		}), []int{0, 1, 2, 3}},
		{"MultiFieldBQ4", bq(func(q *search.BooleanQueryBuilder) {
			q.Add(simpleTerm("w1"), search.SHOULD)
			inner := search.NewBooleanQueryBuilder()
			inner.Add(simpleTerm("xx"), search.MUST_NOT)
		...
		{"MultiFieldBQ5", bq(func(q *search.BooleanQueryBuilder) {
			q.Add(simpleTerm("w1"), search.SHOULD)
			inner := search.NewBooleanQueryBuilder()
		...
		{"MultiFieldBQ6", bq(func(q *search.BooleanQueryBuilder) {
			q.Add(simpleTerm("w1"), search.SHOULD)
			inner := search.NewBooleanQueryBuilder()
		...
		{"MultiFieldBQ7", bq(func(q *search.BooleanQueryBuilder) {
			q.Add(simpleTerm("w1"), search.MUST)
			inner := search.NewBooleanQueryBuilder()
		...
		{"MultiFieldBQ8", bq(func(q *search.BooleanQueryBuilder) {
		...
		{"MultiFieldBQ9", bq(func(q *search.BooleanQueryBuilder) {
			q.Add(simpleTerm("w1"), search.MUST)
			inner := search.NewBooleanQueryBuilder()
			inner.Add(simpleAltTerm("qq"), search.SHOULD)
			cl := search.NewBooleanQueryBuilder()
		...
		{"MultiFieldBQ10", bq(func(q *search.BooleanQueryBuilder) {
			q.Add(simpleTerm("w1"), search.MUST)
			inner := search.NewBooleanQueryBuilder()
			inner.Add(simpleAltTerm("qq"), search.SHOULD)
			cl := search.NewBooleanQueryBuilder()
		...
		{"MultiFieldBQofPQ1", bq(func(q *search.BooleanQueryBuilder) {
			q.Add(search.NewPhraseQuery(0, explField, "w1", "w2"), search.SHOULD)
			q.Add(search.NewPhraseQuery(0, explAltField, "w1", "w2"), search.SHOULD)
		}), []int{0}},
		{"MultiFieldBQofPQ2", bq(func(q *search.BooleanQueryBuilder) {
			q.Add(search.NewPhraseQuery(0, explField, "w1", "w3"), search.SHOULD)
			q.Add(search.NewPhraseQuery(0, explAltField, "w1", "w3"), search.SHOULD)
		}), []int{1, 3}},
		{"MultiFieldBQofPQ3", bq(func(q *search.BooleanQueryBuilder) {
			q.Add(phraseSlop(1, explField, "w1", "w2"), search.SHOULD)
			q.Add(phraseSlop(1, explAltField, "w1", "w2"), search.SHOULD)
		}), []int{0, 1, 2}},
		{"MultiFieldBQofPQ4", bq(func(q *search.BooleanQueryBuilder) {
			q.Add(phraseSlop(1, explField, "w2", "w3"), search.SHOULD)
			q.Add(phraseSlop(1, explAltField, "w2", "w3"), search.SHOULD)
		}), []int{0, 1, 2, 3}},
		{"MultiFieldBQofPQ5", bq(func(q *search.BooleanQueryBuilder) {
			q.Add(phraseSlop(1, explField, "w3", "w2"), search.SHOULD)
			q.Add(phraseSlop(1, explAltField, "w3", "w2"), search.SHOULD)
		}), []int{1, 3}},
		{"MultiFieldBQofPQ6", bq(func(q *search.BooleanQueryBuilder) {
			q.Add(phraseSlop(2, explField, "w3", "w2"), search.SHOULD)
			q.Add(phraseSlop(2, explAltField, "w3", "w2"), search.SHOULD)
		}), []int{0, 1, 3}},
		{"MultiFieldBQofPQ7", bq(func(q *search.BooleanQueryBuilder) {
