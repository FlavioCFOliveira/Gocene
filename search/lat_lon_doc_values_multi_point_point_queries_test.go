...
// TestLatLonDocValuesMultiPointPoint_SameShapeManyTimes verifies
// concrete subclass configuration: shape type, validator, and helpers.
func TestLatLonDocValuesMultiPointPoint_SameShapeManyTimes(t *testing.T) {
	if latLonDocValuesMultiPointShapeType != latLonShapeTypePoint {
		t.Fatalf("shape type: got %v, want %v", latLonDocValuesMultiPointShapeType, latLonShapeTypePoint)
	}
	if latLonDocValuesMultiPointMaxPoints != 4 {
		t.Fatalf("max points: got %d, want %d", latLonDocValuesMultiPointMaxPoints, 4)
	}

	v := newLatLonDocValuesMultiPointValidator(latLonEncoder{})
	if v == nil {
		t.Fatal("newLatLonDocValuesMultiPointValidator returned nil")
	}
	...
	// Verify nextShape returns nil (stub).
	shape := nextLatLonDocValuesMultiPointShape()
	if shape != nil {
		t.Fatalf("expected nil from stub nextShape, got %v", shape)
	}

	// Verify indexable fields helper returns nil (stub).
	fields := createLatLonDocValuesMultiPointIndexableFields("shape", nil)
	if fields != nil {
		t.Fatalf("expected nil from stub, got %v", fields)
	}
}

// TestLatLonDocValuesMultiPointPoint_LowCardinalityShapeManyTimes verifies
...
func TestLatLonDocValuesMultiPointPoint_LowCardinalityShapeManyTimes(t *testing.T) {
	...
	_ = newLatLonDocValuesMultiPointValidator(latLonEncoder{})
	_ = createLatLonDocValuesMultiPointIndexableFields("shape", nil)
}

// TestLatLonDocValuesMultiPointPoint_RandomTiny verifies validator
// construction and stub return values.
...
// TestLatLonDocValuesMultiPointPoint_RandomMedium verifies shape type
// and max points constant.
...
// TestLatLonDocValuesMultiPointPoint_RandomBig verifies shape type,
// validator, and the magnitude constant.
