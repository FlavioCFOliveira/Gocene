...
func TestBooleanMinShouldMatch_AllOptional(t *testing.T) {
	...
	verifyNrHits(t, searcher, bq.Build(), 2)
}
...
func TestBooleanMinShouldMatch_OneReqAndSomeOptional(t *testing.T) {
	...
	verifyNrHits(t, searcher, bq.Build(), 5)
}
...
func TestBooleanMinShouldMatch_OneProhibAndSomeOptional(t *testing.T) {
	...
	verifyNrHits(t, searcher, bq.Build(), 1)
}
...
func TestBooleanMinShouldMatch_MinHigherThanNumOptional(t *testing.T) {
	...
	verifyNrHits(t, searcher, bq.Build(), 0)
}
...
func TestBooleanMinShouldMatch_RewriteMSM1(t *testing.T) {
	...
	top1, err := searcher.Search(bq1.Build(), 100)
	if err != nil {
		t.Fatalf("Search failed: %v", err)
	}

	top2, err := searcher.Search(bq2.Build(), 100)
	if err != nil {
		t.Fatalf("Search failed: %v", err)
	}
...
func TestBooleanMinShouldMatch_FlattenInnerDisjunctions(t *testing.T) {
	...
	verifyNrHits(t, searcher, bq1.Build(), 1)
	...
	bq2.Add(inner.Build(), search.SHOULD)
	bq2.Add(search.NewTermQuery(index.NewTerm("data", "2")), search.MUST)

	verifyNrHits(t, searcher, bq2.Build(), 0)
}
...
func TestBooleanMinShouldMatch_Basics(t *testing.T) {
	...
	if bq.Build().GetMinimumNumberShouldMatch() != 2 {
		t.Errorf("Expected minShouldMatch=2, got %d", bq.Build().GetMinimumNumberShouldMatch())
	}

	// Test that Equals considers minShouldMatch
	bq2 := search.NewBooleanQueryBuilder()
	bq2.SetMinimumNumberShouldMatch(3)
	if bq.Build().Equals(bq2.Build()) {
...
func TestBooleanMinShouldMatch_RandomQueries(t *testing.T) {
	...
	for i := 0; i < num; i++ {
		...
		opt := 0
		for _, c := range q2.Build().Clauses() {
			if c.Occur() == search.SHOULD {
				opt++
			}
		}
		...
		top1, err := searcher.Search(q1.Build(), 100)
		if err != nil {
			t.Fatalf("iter %d Search q1: %v", i, err)
		}
		top2, err := searcher.Search(q2.Build(), 100)
		if err != nil {
			t.Fatalf("iter %d Search q2: %v", i, err)
		}
		assertSubsetOfSameScores(t, q2.Build(), top1, top2)
	}
}
...
func bmsmRandBoolQuery(rng *rand.Rand, allowMust bool, level int, field string, vals []string) *search.BooleanQueryBuilder {
	bq := search.NewBooleanQueryBuilder()
	...
	for i := 0; i < numClauses; i++ {
		...
		if level > 0 && rng.Intn(10) >= 5 {
			q = bmsmRandBoolQuery(rng, allowMust, level-1, field, vals).Build()
		} else {
