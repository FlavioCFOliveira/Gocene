...
func (q *randomApproximationQuery) Rewrite(searcher *search.IndexSearcher) (search.Query, error) {
...
func (s *fixedScorerSupplier) Cost() int64                        { return s.scorer.Iterator().Cost() }
func (s *fixedScorerSupplier) SetTopLevelScoringClause() error    { return nil }
...
func newRandomApproximationScorer(inner search.Scorer, rng *rand.Rand) *randomApproximationScorer {
	...
	s.approx = &randomApproximation{rng: rng, disi: inner.Iterator(), doc: -1}
...
// Iterator mirrors RandomApproximationScorer.iterator():
// TwoPhaseIterator.asDocIdSetIterator(twoPhaseView).
func (s *randomApproximationScorer) Iterator() search.DocIdSetIterator { return s.disi }
...
func (q *assertingQuery) Rewrite(searcher *search.IndexSearcher) (search.Query, error) {
	rewritten, err := q.in.Rewrite(searcher)
	if err != nil {
		return nil, err
	}
	if rewritten == q.in {
...
func (q *assertingQuery) Equals(other spi.Query) bool {
...
func (q *assertingQuery) CreateWeight(searcher *search.IndexSearcher, scoreMode search.ScoreMode, boost float32) (search.Weight, error) {
