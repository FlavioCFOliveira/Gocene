...
// Lucene99MemorySegmentFloatVectorScorerSupplier.java).
//
// In Lucene, this sealed class is the JDK-21 Panama/MemorySegment fast path
// for HNSW float-vector scoring. It is selected only when the underlying
...
// each delegating to MemorySegmentBulkVectorOps for SIMD-vectorised kernels
// (single-vector and 4-way bulk score) over the raw segment bytes.
//
...
//     max inner product), which operate on []float32 backed by the same
//     mmap-backed bytes that MMapDirectory hands out (see
