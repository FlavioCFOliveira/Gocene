...
// and then retrieve slices (util.BytesRef) into it using fill.
//
...
// 1<<blockBits must be bigger than biggest single util.BytesRef slice that will be pulled.
...
func (p *PagedBytes) Copy(input IndexInput, byteCount int64) error {
	if p.frozen {
...
// CopyBytesRef copies a util.BytesRef into this PagedBytes and sets the out util.BytesRef to the result.
// Do not use this if you will use Freeze(true). This only supports bytes.Length <= blockSize.
func (p *PagedBytes) CopyBytesRef(bytes *util.BytesRef, out *util.BytesRef) error {
	if p.frozen {
		return fmt.Errorf("cannot copy after freeze")
	}
	if bytes == nil {
...
func (p *PagedBytes) CopyUsingLengthPrefix(bytes *util.BytesRef) (int64, error) {
	if p.frozen {
...
func (p *PagedBytes) GetDataOutput() *PagedBytesDataOutput {
	out := &PagedBytesDataOutput{pagedBytes: p}
	out.InitBase()
	return out
}

// FillSlice fills the given util.BytesRef with a slice starting at start with the given length.
// Slices spanning more than two blocks are not supported.
func (r *Reader) FillSlice(b *util.BytesRef, start int64, length int) error {
	if length < 0 {
...
func (r *Reader) Fill(b *util.BytesRef, start int64) error {
	index := int(start >> r.blockBits)
	offset := int(start & int64(r.blockMask))
...
func (in *PagedBytesDataInput) InitBase() {
	in.Core = in
}

// ReadFloats reads len floats into dst.
func (in *PagedBytesDataInput) ReadFloats(dst []float32, offset, len int) error {
	// PagedBytesDataInput is a simple sequential reader.
	// For absolute reads, one should use the RandomAccessInput interface.
	// Here we just read them sequentially.
	for i := 0; i < len; i++ {
		b, err := in.ReadInt()
		if err != nil {
			return err
		}
		dst[offset+i] = math.Float32frombits(uint32(b))
	}
	return nil
}
...
func (in *PagedBytesDataInput) ReadBytesN(n int) ([]byte, error) {
	buf := make([]byte, n)
	if err := in.ReadBytes(buf, 0, n); err != nil {
		return nil, err
	}
	return buf, nil
}
...
func (out *PagedBytesDataOutput) InitBase() {
	out.primitive = out
}
...
func (out *PagedBytesDataOutput) WriteBytesN(b []byte, length int) error {
	...
	return out.WriteBytes(b, 0, length)
}
...
var _ DataInput = (*PagedBytesDataInput)(nil)
var _ DataOutput = (*PagedBytesDataOutput)(nil)
...
func (out *PagedBytesDataOutput) CopyBytes(input DataInput, numBytes int64) error {
	remaining := numBytes
