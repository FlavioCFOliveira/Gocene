...
func WriteHeader(out DataOutput, codec string, version int32) error {
	...
	if err := WriteInt32(out, CODEC_MAGIC); err != nil {
		return err
	}
	if err := WriteString(out, codec); err != nil {
		return err
	}
	return WriteInt32(out, version)
}
...
func writeSuffix(out IndexOutput, suffix string) error {
	if len(suffix) > 255 {
...
func CheckHeader(in IndexInput, codec string, minVersion, maxVersion int32) (int32, error) {
	magic, err := ReadInt32(in)
	if err != nil {
		return 0, err
	}
...
func CheckIndexHeader(in IndexInput, codec string, minVersion, maxVersion int32, expectedId []byte, expectedSuffix string) (int32, error) {
	...
	id := make([]byte, 16)
	if err := in.ReadBytes(id, 0, 16); err != nil {
		return 0, err
	}
	...
	suffixBytes := make([]byte, int(suffixLen))
	if err := in.ReadBytes(suffixBytes, 0, int(suffixLen)); err != nil {
		return 0, err
	}
...
func WriteFooter(out IndexOutput) error {
	if err := WriteInt32(out, FOOTER_MAGIC); err != nil {
		return err
	}
	if err := WriteInt32(out, 0); err != nil {
		return err
	}
	return WriteCRC(out)
}
...
func WriteCRC(out IndexOutput) error {
	if cw, ok := out.(checksumWriter); ok {
		...
		return WriteInt64(out, int64(checksum))
	}
	return fmt.Errorf("output does not support checksums")
}

// ReadCRC reads a checksum from the input.
func ReadCRC(in IndexInput) (int64, error) {
	checksum, err := ReadInt64(in)
	if err != nil {
		return 0, err
	}
