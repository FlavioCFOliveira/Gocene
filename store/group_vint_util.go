...
func ReadGroupVInts(in DataInput, dst []int32, limit int) error {
	if limit < 0 {
...
func readGroupVInt32(in DataInput, dst []int32, offset int) error {
	if offset+4 > len(dst) {
	...
	if iin, ok := in.(IndexInput); ok {
		if rin, ok2 := in.(RandomAccessInput); ok2 {
...
func ReadGroupVIntsBaseline(in DataInput, dst []int32, limit int) error {
	if limit < 0 {
		return fmt.Errorf("group vint: negative limit %d", limit)
	}
	if limit > len(dst) {
...
func readGroupVInt32Baseline(in DataInput, dst []int32, offset int) error {
	if offset+4 > len(dst) {
		return fmt.Errorf("group vint: dst overflow at offset %d (len=%d)", offset, len(dst))
	}
	flag, err := in.ReadByte()
...
func readIntInGroup(in DataInput, numBytesMinus1 int) (uint32, error) {
	n := numBytesMinus1 + 1
	var v uint32
	for b := 0; b < n; b++ {
		x, err := in.ReadByte()
...
func ReadGroupVIntsInt64(in DataInput, dst []int64, limit int) error {
	if limit < 0 {
		return fmt.Errorf("group vint: negative limit %d", limit)
	}
	if limit > len(dst) {
...
//
// Deprecated: only kept for backwards-compatible codecs.
func ReadGroupVIntInt64(in DataInput, dst []int64, offset int) error {
	return readGroupVInt64(in, dst, offset)
}

func readGroupVInt64(in DataInput, dst []int64, offset int) error {
	if offset+4 > len(dst) {
		return fmt.Errorf("group vint: dst overflow at offset %d (len=%d)", offset, len(dst))
	}
	flag, err := in.ReadByte()
