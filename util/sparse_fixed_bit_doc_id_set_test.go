...
// DocIdSetIterator.intoBitSet(upTo, dest, offset). Gocene's
// DocIdSetIterator surface does not yet expose IntoBitSet (see
// codecs/lucene90/indexed_disi.go lines 62, 130, 563 for the standing gap
// and ErrIntoBitSetNotSupported). Build the wiring verbatim so the test
// "just works" once IntoBitSet lands on the iterator surface in
// search/doc_id_set_iterator.go (and on BitSetIterator).
func TestSparseFixedBitDocIdSet_IntoBitSet(t *testing.T) {
	...
	// expected list, which is the functional equivalent of IntoBitSet.
	var got []int
	for {
		doc, err := it.NextDoc()
		if err != nil {
...
// that Advance and NextDoc navigate the two-set-bit case correctly.
