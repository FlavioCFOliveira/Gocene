...
- **Live graph, verified 2026-09-23 (module tier synchronised to commit
  `a4769f4d`):** **210 363 nodes, 333 527 edges, 29 labels defined (28 populated —
...
`git archive` snapshots of commits `0d52b534` and `a4769f4d`** — the post-sync census and the
two-direction diff find 0 differences (§ 6). `PORTED_TO` is **populated** over 10
endpoint pairs: 23 535 edges, derived from measured evidence (§ 7, The PORTED_TO derivation) and verified by
...
two-direction identity and property diff against the inventory of the `a4769f4d`
...
snapshot of commit `a4769f4d` (2026-09-23). It is **loaded and verified**: every
...
- **Identity:** `path` — repo-relative. **Population: 4959** (4860 Go + 99 non-Go).
- `language` STRING — `go` (4860) | `java` (80) | `python` (9) | `shell` (7) |
  `makefile` (2) | `gomod` (1).
- `role` STRING — Go files: `production` (2976) | `test` (1877) | `example` (5) |
...
- `importPath` STRING — for Go files, the declared package's importPath (all 4860
...
- **Identity:** `qn` — `<importPath>.Name`, with the tier disambiguator where the
  natural key collides. **Population: 7727.**
- `name` STRING — 7009 distinct (90.7 % selective).
- `kind` STRING — `struct` (6157) | `interface` (950) | `defined` (346) |
  `alias` (274).
...
- **Identity:** `qn` — `<importPath>.Name`, with the tier disambiguator; `init`
  ...
  **Population: 23824.**
- `name` STRING — 22775 distinct (95.6 % selective).
- `kind` STRING — `function` (11689) | `test` (11898) | `init` (84) |
  `benchmark` (144) | `fuzz` (7) | `example` (2).
- `line` INTEGER.
...
- **Identity:** `sig` — `<importPath>.<receiver>.<name>` (type arguments erased in
  ...
  **Population: 33054** (30259 concrete + 2795 interface specs).
- `name` STRING — **not an identity**: 7785 distinct (23.6 % selective; `String`
  alone appears on 707 receivers).
...
- `recvPtr` BOOLEAN — pointer receiver; true for 28987 of the 30259 concrete.
- `interface` BOOLEAN — true for the 2795 interface method specs.
- `ifaceQN` STRING — interface specs only: the interface's QN (all 2795 resolve to
  a declared type).
- `line` INTEGER.
...
- **Identity:** `key` — `<typeQN>.<name-part>`, with the tier disambiguator; the
  ...
  **Population: 21405** (19439 + 1966 embedded).
- `name` STRING — **not an identity**: 6979 distinct (32.6 % selective).
...
- **Identity:** `qn` — `<importPath>.Name`, with the tier disambiguator.
  **Population: 3286.**
- `name` STRING — 2988 distinct (90.9 % selective).
- `type` STRING — the declared type; empty for the 2887 untyped constants.
- `line` INTEGER.

### `GoceneVariable`
- **Identity:** `qn` — `<importPath>.Name`, with the tier disambiguator.
  **Population: 3611.**
- `name` STRING — **not an identity**: 1705 distinct (47.2 % selective); the 1817
...
- `type` STRING — the declared type; empty for the 1539 variables whose type is
...
- `origin` STRING — `qualified` (13: the reference text carries a package
  qualifier) | `unresolved` (0 at `a4769f4d`: a bare simple name whose package
...
- **Identity:** `importPath`. **Population: 0** at `a4769f4d` — the label, its
  ...
  `go list -e -test ./...` over the `a4769f4d` snapshot reports no missing package.
...
| `CONTAINS_FILE` | `GocenePackage` → `GoceneFile` | the file's production package (Go files: the declared package; non-Go files: the production package of the directory) | 4870 |
| `CONTAINS_FILE` | `GoceneModule` → `GoceneFile` | a file in a directory with no production package (`docs/`, `scripts/`, `tools/`) | 89 |
| `DECLARES_TYPE` | `GoceneFile` → `GoceneType` | a type declared by this file | 7727 |
| `DECLARES_FUNCTION` | `GoceneFile` → `GoceneFunction` | a package-level function of any kind | 23824 |
| `DECLARES_CONSTANT` | `GoceneFile` → `GoceneConstant` | a package-level constant | 3286 |
| `DECLARES_VARIABLE` | `GoceneFile` → `GoceneVariable` | a package-level variable | 3611 |
| `DECLARES_METHOD` | `GoceneFile` → `GoceneMethod` | a method declared by this file — concrete or interface spec | 33054 |
| `DECLARES_METHOD` | `GoceneType` → `GoceneMethod` | a member method of this type — concrete with a resolvable receiver, or an interface spec of this interface | 33065 |
| `DECLARES_FIELD` | `GoceneType` → `GoceneField` | a member field or embedded field of this type | 21405 |
...
| `IMPORTS` | `GoceneFile` → `GocenePackage` | an import of a package inside the module; edge property `blank` BOOLEAN, always present — true for the 90 `_` imports | 6438 |
| `IMPORTS` | `GoceneFile` → `GoceneExternalPackage` | an import of a package outside the module; edge property `blank` BOOLEAN, present only on the 8 `_` imports (`true`) | 7812 |
...
Every `GoceneFile` carries exactly one `CONTAINS_FILE` edge (4870 + 89 = 4959), and
every `IMPORTS` edge from a Go file targets exactly one of the three package labels
(6438 + 7812 + 0 = 14 250; the in-module targets were 6 440 raw inventory rows,
...
belongs to every QN that shares the natural identity — measured 33 041 methods
with exactly one type owner, 12 with two (33 065 type→method edges), and 1 with
...
sync to `a4769f4d`, each equal to the sync plan's final row count for that pair (§ 7, The
PORTED_TO derivation).

| Predicate | From → To | Asserts | Population |
|---|---|---|---:|
| `PORTED_TO` | `LuceneRelease` → `GoceneModule` | the release is ported to the module — the root of the port | 1 |
| `PORTED_TO` | `LuceneFile` → `GoceneFile` | this Lucene file is ported in this Go file (a Go source comment references the Lucene path) | 766 |
| `PORTED_TO` | `LucenePackage` → `GocenePackage` | this Lucene package is ported to this Go package | 401 |
| `PORTED_TO` | `LuceneClass` → `GoceneType` | this Lucene class is ported to this Go type | 2 392 |
| `PORTED_TO` | `LuceneMethod` → `GoceneMethod` | this method or constructor is ported to this method — Java overloads map many-to-one onto the single Go method | 9 314 |
| `PORTED_TO` | `LuceneField` → `GoceneField` | this Lucene field is ported to this Go field (non-constant members only) | 5 283 |
...
| `PORTED_TO` | `LuceneMethod` → `GoceneFunction` | this constructor, static method, test method or test helper is ported to this package-level Go function (`<init>` → `NewX`; test method `testFoo()` → `TestFoo`, `Test<Class>_Foo` or `Test<Class>_testFoo`; static method `foo(…)` → `Foo`) | 4 785 |
...
**0 of the 210 363 nodes and 0 of the 333 527 edges lack `gitCommit`/`gitDate`**.
...
- **Gocene tier** — an element carries the commit that **last changed its source
  ...
  `2026-09-08` (52 765 of 98 295 nodes, 112 691 of 147 487 edges); the tier carries
  93 distinct `gitCommit` values on nodes and 99 on edges.
- **`PORTED_TO`** — the 6 688 edges of the original 2026-09-11 derivation left
...
| Gocene tier (§ 1, § 2) | 12 labels, 11 predicates over 16 endpoint pairs | **98 295 nodes, 147 487 edges over 15 populated pairs** — first loaded at `dd61538c`, resynchronised 2026-09-23 to the `git archive` snapshot of `f008c200`, synchronised incrementally the same day to the snapshots of `0d52b534` and `a4769f4d`; post-sync census and two-direction identity, property and edge diff over all 12 labels: 0 differences |
| `PORTED_TO` (§ 2) | 10 endpoint pairs | **23 535 edges over the 10 pairs** — materialised 2026-09-11 by measured derivation, re-derived 2026-09-23 over the whole `f008c200` snapshot, for the files changed up to `0d52b534`, over the whole graph with the extended rules, and for the files changed up to `a4769f4d` (§ 7): write-time counter audit exact, live set equal to the plan, anchor audit 0 violations, `duplicate` flags consistent with the edges, provenance 100 % |
...
reconciled against `go list -e -test ./...` over the `a4769f4d` snapshot (341 import
...
| A file that fails to parse in the scanner keeps a `GoceneFile` node with zero declaration edges | its declarations cannot be extracted — visible by query, not hidden; measured at `a4769f4d`: 0 such files (10 at `dd61538c`) |
...
| `GoceneExternalType` mixes genuinely external references with dangling internal ones | 0 `origin: 'unresolved'` names at `a4769f4d` (the bare name `Sorter` left with its last reference); the reference text is preserved as written — resolving it is a repair, not a re-identification |
