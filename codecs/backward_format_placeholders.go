...
// for backward-compatibility codec names whose full read path has not been
// ported yet. It rejects writes (ErrReadOnlyFormat) and returns a descriptive
// error on reads, allowing format name resolution without panicking.
//
// It is used by backward_codecs sub-packages to ensure PostingsFormatByName
// resolves historic format names.
...
func (f *readOnlyPostingsFormatStub) FieldsProducer(_ *SegmentReadState) (FieldsProducer, error) {
	return nil, fmt.Errorf("%s: FieldsProducer not yet implemented (backward format not ported)", f.Name())
}
...
func (f *readOnlyKnnVectorsFormatStub) FieldsReader(_ *SegmentReadState) (KnnVectorsReader, error) {
	return nil, fmt.Errorf("%s: FieldsReader not yet implemented (backward format not ported)", f.Name())
}
