...
// org.apache.lucene.analysis.ko.dict.ConnectionCostsBuilder from Apache
// Lucene 10.4.0.
...
func (ConnectionCostsBuilder) Build(r io.Reader) (*morph.ConnectionCosts, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("connectionCostsBuilder: invalid forwardSize: %w", err)
	}
	bwd, err := strconv.Atoi(dims[1])
	if err != nil {
		return nil, fmt.Errorf("connectionCostsBuilder: invalid backwardSize: %w", err)
	}
