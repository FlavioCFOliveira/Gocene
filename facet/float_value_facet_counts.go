...
// FloatValueFacetCounts is a Facets implementation that computes counts for all unique float values.
//
// This is the Go port of Lucene's org.apache.lucene.facet.FloatValueFacetCounts.
type FloatValueFacetCounts struct {
	hashCounts  map[float32]int
	initialized bool
	field       string
	totCount    int
}

// NewFloatValueFacetCounts creates a new FloatValueFacetCounts.
func NewFloatValueFacetCounts(field string, hits *FacetsCollector) error {
	f := &FloatValueFacetCounts{
		field: field,
	}
...
func (f *FloatValueFacetCounts) initializeCounters() {
	if f.initialized {
		return
	}
	f.hashCounts = make(map[float32]int)
	f.initialized = true
}

func (f *FloatValueFacetCounts) increment(value float32) {
	f.hashCounts[value]++
}

func (f *FloatValueFacetCounts) count(valueSource search.FloatValueSource, matchingDocs []*MatchingDocs) error {
...
func (f *FloatValueFacetCounts) GetAllChildren(dim string, path ...string) (*FacetResult, error) {
	if dim != f.field {
...
func (f *FloatValueFacetCounts) GetTopChildren(topN int, dim string, path ...string) (*FacetResult, error) {
...
func (f *FloatValueFacetCounts) GetSpecificValue(dim string, path ...string) (interface{}, error) {
	panic("unsupported")
}

func (f *FloatValueFacetCounts) GetAllDims(topN int) ([]*FacetResult, error) {
...
func (f *FloatValueFacetCounts) GetTopDims(topNDims, topNChildren int) ([]*FacetResult, error) {
