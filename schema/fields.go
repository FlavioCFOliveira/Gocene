...
// NewMemoryFieldIterator builds a MemoryFieldIterator over the given
// field names. The iterator is positioned before the first name; the
// first call to Next returns names[0]. The slice is retained by
// reference — callers must not mutate it after handing it off.
func NewMemoryFieldIterator(names []string) *MemoryFieldIterator {
	return &MemoryFieldIterator{fields: names, index: -1}
}
...
// FieldsList exposes the wrapped sub-Fields slice for callers that need
// to drive a custom merge over the same backing storage. The returned
// slice aliases the internal storage — callers must not mutate it.
