...
// FieldInfos, or "" if none. Until parent-document blocks are wired
// into the indexing chain, this returns "" unconditionally. Mirrors
...
// FieldInfos returns the underlying *FieldInfos accumulated so far. The
// returned value aliases the builder's internal state; callers must not
// retain it across further Add / AddFromOptions / Build calls.
//
// Exposed so that consumers in other packages (notably the indexing
// chain) can probe the in-progress FieldInfos without going through
// Build, which would freeze the builder.
