...
// Package schema holds the structural types that both the index/ package
// and the codecs/ family operate on. It is a leaf package — it depends
// only on the standard library and on the store/ primitive package — so
// that it can be imported from anywhere in the Gocene tree without
// creating cycles.
//
// schema is the canonical declaration site for SegmentInfo and the sort
// metadata it carries. The index/ package re-exports these types via Go
// type aliases (type X = schema.X) for source-level backward
// compatibility with callers that historically reached for index.X.
//
// This package is part of the SPI unification work tracked under rmp
// #4669 — see schema/doc.go for the longer rationale.
package schema
