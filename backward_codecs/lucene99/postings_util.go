...
// BlockSize is the postings block size for the Lucene 9.9 format.
...
//
// Port of org.apache.lucene.backward_codecs.lucene99.PostingsUtil#readVIntBlock
// (Lucene 10.4.0).
func ReadVIntBlock(
	...
	if num < 0 {
		return fmt.Errorf("lucene99 postings: negative num %d", num)
	}
	if num > len(docBuffer) {
		return fmt.Errorf("lucene99 postings: docBuffer too short: len=%d num=%d", len(docBuffer), num)
	}
	if indexHasFreq && decodeFreq && num > len(freqBuffer) {
		return fmt.Errorf("lucene99 postings: freqBuffer too short: len=%d num=%d", len(freqBuffer), num)
	}
...
//
// Port of org.apache.lucene.backward_codecs.lucene99.PostingsUtil#writeVIntBlock
// (Lucene 10.4.0).
func WriteVIntBlock(
