...
func roundtripVInt(t *testing.T, docs, freqs []int64, num int, writeFreqs bool) ([]int64, []int64) {
	t.Helper()
	...
	dc := append([]int64(nil), docs[:num]...)
	fc := append([]int64(nil), freqs[:num]...)
...
func TestPostingsUtil_IntegerOverflow(t *testing.T) {
	const delta = 1 << 30
	for _, size := range []int{1, 4, BlockSize / 2, BlockSize} {
		docs := make([]int64, size)
		freqs := make([]int64, size)
		docs[0] = delta
...
func TestPostingsUtil_Roundtrip(t *testing.T) {
	n := 6
	docs := []int64{5, 10, 3, 7, 1, 2}
	freqs := []int64{1, 2, 1, 3, 1, 4}
...
func TestPostingsUtil_NegativeNum(t *testing.T) {
	...
	if err := WriteVIntBlock(out, []int64{}, []int64{}, -1, false); err == nil {
		t.Error("WriteVIntBlock(-1): expected error")
	}
	in, _ := dir.OpenInput("e.dat", store.IOContext{})
	defer func() { _ = in.Close() }()
	if err := ReadVIntBlock(in, []int64{}, []int64{}, -1, false, false); err == nil {
