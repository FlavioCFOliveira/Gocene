...
func TestSortedSetDocValuesWriter_FlushSingleton(t *testing.T) {
	...
	err := w.flushToCallback(3, nil, func(fi *FieldInfo, v SortedSetDocValues) error {
		if fi.Name() != "f" {
...
func TestSortedSetDocValuesWriter_FlushSorted(t *testing.T) {
	...
	err := w.flushToCallback(maxDoc, sortMap, func(_ *FieldInfo, v SortedSetDocValues) error {
		for {
			d, err := v.NextDoc()
			if err != nil {
				return err
...
func TestSortedSetDocValuesWriter_RejectInvalid(t *testing.T) {
	...
	t.Run("nil consumer", func(t *testing.T) {
		...
		err := w.Flush(flushTestState(1), nil, nil)
		if err == nil {
