...
func TestNewConcurrentFacetsAccumulator(t *testing.T) {
	tests := []struct {
		name        string
		config      *FacetsConfig
		wantErr     bool
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			acc, err := NewConcurrentFacetsAccumulator(tt.config)
			if tt.wantErr {
				if err == nil {
			...
			if acc.GetNumWorkers() != 4 {
				t.Errorf("expected num workers 4, got %d", acc.GetNumWorkers())
			}
		})
	}
}

func TestConcurrentFacetsAccumulator_Accumulate(t *testing.T) {
	config := NewFacetsConfig()
	acc, err := NewConcurrentFacetsAccumulator(config)
	if err != nil {
...
func TestConcurrentFacetsAccumulator_ConcurrentAccumulate(t *testing.T) {
	config := NewFacetsConfig()
	acc, err := NewConcurrentFacetsAccumulator(config)
	if err != nil {
		t.Fatalf("failed to create accumulator: %v", err)
	...
	// Test concurrent accumulation
	var wg sync.WaitGroup
	numGoroutines := 10
	numResultsPerGoroutine := 100

	for i := 0; i < numGoroutines; i++ {
		wg.Add(1)
		go func(id int) {
			defer wg.Done()
			for j := 0; j < numResultsPerGoroutine; j++ {
				result := NewFacetResult("category")
				result.AddLabelValue(NewLabelAndValue("category/electronics", 1))
	...
	// Verify total count
	electronicsOrd := acc.getOrCreateOrdinal("category/electronics")
	expectedCount := int64(numGoroutines * numResultsPerGoroutine)
	if acc.GetCount(electronicsOrd) != expectedCount {
		t.Errorf("expected electronics count %d, got %d", expectedCount, acc.GetCount(electronicsOrd))
	}
}

func TestConcurrentFacetsAccumulator_GetTopChildren(t *testing.T) {
	config := NewFacetsConfig()
	acc, err := NewConcurrentFacetsAccumulator(config)
	if err != nil {
		t.Fatalf("failed to create accumulator: %v", err)
...
func TestConcurrentFacetsAccumulator_GetAllChildren(t *testing.T) {
	config := NewFacetsConfig()
	acc, err := NewConcurrentFacetsAccumulator(config)
	if err != nil {
		t.Fatalf("failed to create accumulator: %v", err)
...
func TestConcurrentFacetsAccumulator_GetSpecificValue(t *testing.T) {
	config := NewFacetsConfig()
	acc, err := NewConcurrentFacetsAccumulator(config)
	if err != nil {
		t.Fatalf("failed to create accumulator: %v", err)
...
func TestConcurrentFacetsAccumulator_GetDimensions(t *testing.T) {
	config := NewFacetsConfig()
	acc, err := NewConcurrentFacetsAccumulator(config)
	if err != nil {
		t.Fatalf("failed to create accumulator: %v", err)
...
func TestConcurrentFacetsAccumulator_Reset(t *testing.T) {
	config := NewFacetsConfig()
	acc, err := NewConcurrentFacetsAccumulator(config)
	if err != nil {
		t.Fatalf("failed to create accumulator: %v", err)
...
func TestConcurrentFacetsAccumulator_IsEmpty(t *testing.T) {
	config := NewFacetsConfig()
	acc, err := NewConcurrentFacetsAccumulator(config)
	if err != nil {
		t.Fatalf("failed to create accumulator: %v", err)
...
func TestConcurrentFacetsAccumulator_GetCount(t *testing.T) {
	config := NewFacetsConfig()
	acc, err := NewConcurrentFacetsAccumulator(config)
	if err != nil {
		t.Fatalf("failed to create accumulator: %v", err)
...
func TestConcurrentFacetsAccumulator_SetNumWorkers(t *testing.T) {
	config := NewFacetsConfig()
	acc, err := NewConcurrentFacetsAccumulator(config)
	if err != nil {
		t.Fatalf("failed to create accumulator: %v", err)
	...
	// Default should be 4
	if acc.GetNumWorkers() != 4 {
		t.Errorf("expected num workers 4 by default")
	}

	// Set to 8
	acc.SetNumWorkers(8)
	if acc.GetNumWorkers() != 8 {
		t.Errorf("expected num workers 8")
	}

	// Set to 0 (should not change)
	acc.SetNumWorkers(0)
	if acc.GetNumWorkers() != 8 {
		t.Errorf("expected num workers to remain 8")
	}
}

func TestConcurrentFacetsAccumulator_SetHierarchical(t *testing.T) {
	config := NewFacetsConfig()
	acc, err := NewConcurrentFacetsAccumulator(config)
	if err != nil {
		t.Fatalf("failed to create accumulator: %v", err)
...
func TestConcurrentFacetsAccumulator_SetMaxCategories(t *testing.T) {
	config := NewFacetsConfig()
	acc, err := NewConcurrentFacetsAccumulator(config)
	if err != nil {
		t.Fatalf("failed to create accumulator: %v", err)
...
func TestConcurrentFacetsAccumulatorFactory(t *testing.T) {
	config := NewFacetsConfig()
	factory := NewConcurrentFacetsAccumulatorFactory(config)

	acc, err := factory.SetNumWorkers(8).CreateAccumulator()
	if err != nil {
		t.Errorf("unexpected error: %v", err)
		return
	}
	...
	if acc.GetNumWorkers() != 8 {
		t.Errorf("expected num workers 8, got %d", acc.GetNumWorkers())
	}
}

func TestConcurrentFacetsAccumulatorBuilder(t *testing.T) {
	config := NewFacetsConfig()

	acc, err := NewConcurrentFacetsAccumulatorBuilder(config).
		SetNumWorkers(16).
	...
	if acc.GetNumWorkers() != 16 {
		t.Errorf("expected num workers 16, got %d", acc.GetNumWorkers())
	}
...
func TestConcurrentFacetsAccumulatorBuilder_BuildError(t *testing.T) {
	// Test with nil config
	_, err := NewConcurrentFacetsAccumulatorBuilder(nil).Build()
	if err == nil {
		t.Errorf("expected error with nil config")
	}
}

func TestConcurrentFacetsAccumulator_MaxCategoriesLimit(t *testing.T) {
	config := NewFacetsConfig()
	acc, err := NewConcurrentFacetsAccumulator(config)
	if err != nil {
		t.Fatalf("failed to create accumulator: %v", err)
...
func TestConcurrentFacetsAccumulator_AccumulateFromMatchingDocs(t *testing.T) {
	config := NewFacetsConfig()
	acc, err := NewConcurrentFacetsAccumulator(config)
	if err != nil {
		t.Fatalf("failed to create accumulator: %v", err)
